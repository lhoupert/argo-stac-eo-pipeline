...
> The crontab line moves into the cluster. Nothing about the work changes — only the
...
CronWorkflow (0 3 * * *) ─▶ ensure-collection ─▶ ingest ──▶ MinIO (assets)
   ...
   the same env-driven config                     └─ FAIL_ONCE: fail once, then succeed
```
...
`src/eo_ingest/ingest.py` is byte-identical to its rung-0 form. The retry lives in the
