...
the point of rung 4: a self-healing pipeline still needs a window onto what it healed.

## Verified live (2026-06-11)

`make demo STAGE=04` produced a report showing `synthetic-aurora-veil` at 0 gaps (all ✅) after
its rung-3 repair, `synthetic-tidal-glass` still at 4 ⬜, and 1 failed-then-retried attempt read
from the durable archive (it survived workflow GC — that's why the archive exists).

Repair a collection (`make demo STAGE=03 ARGS="-p collection=synthetic-tidal-glass"`) and re-run this report: its ⬜ flip to ✅.

## Why no Prometheus / Grafana here

The report is sourced entirely from the Argo Workflows API and the STAC logbook — systems
the pipeline already runs, keeping the demo small and dependency-light. A production-grade
extension would add Grafana and a titiler coverage map for a richer dashboard, running the same
workflows unchanged; that is not part of this demo.
