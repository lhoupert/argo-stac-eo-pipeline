...
> instead of one at a time, with a cap on concurrency.

_Ladder: rung 2 of 5 — you are here._
...
Per-pod startup overhead, on one node. Each ingest is ~2 s of simulated work plus a few seconds
of pod scheduling/pull/init — and a single kind node can't bring up 10 pods' worth of startup in
parallel. So the effective parallelism is below the cap, and the ideal 10× erodes to a measured
~6×. The cap is a politeness ceiling, not a throughput guarantee — it protects the upstream
source (and your one node), and the speedup you actually get is whatever the slowest shared
resource allows. Bigger per-item cost (real IO) pushes the ratio back toward the cap.
...
byte-identical to its rung-1 form.
