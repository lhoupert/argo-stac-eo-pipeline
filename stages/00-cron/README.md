...
This rung runs the one image (the frozen unit of work) on a schedule against a local MinIO,
with no Kubernetes and no catalog. It is deliberately fragile — its job is to make the problems
visible so the later rungs have something to fix.
...
Rung 1 runs the same image, unchanged, under Argo Workflows — and the ingest starts registering
each item into a STAC API (the logbook). You gain orchestration (retries, history, a UI) and a
catalog you can look at. "Nowhere to look at 3am" becomes "open the logbook."
