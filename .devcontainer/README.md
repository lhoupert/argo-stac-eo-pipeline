...
This folder ships the ladder toolchain — `uv`, `kind`, `kubectl`, `argo`, and Docker-in-Docker —
version-pinned so a Codespace behaves like a maintainer's laptop.

Most people never touch the commands below: open the repo in VS Code ("Reopen in Container") or
launch a GitHub Codespace, and everything here is applied automatically. See
...
Useful for a quick "does the Codespace path still work?" check.
...
## Driving the ladder from the terminal
...
Each `devcontainer exec … bash -lc '…'` runs one command inside the container. Working
interactively? Open a shell with `devcontainer exec --workspace-folder . bash` and run the
`make …` targets normally.
...
- Sizing: steps 1–5 are light, but step 6 runs a *nested* Kubernetes cluster and needs the
  ...
  [sizing table](../README.md#dev-container--codespaces).
