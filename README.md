...
Folder number == rung number. Each rung is independently runnable and adds one idea.

| Rung | Stage | What it is |
|------|-------|-----------|
...
Rungs 0–4 run end-to-end on a local `kind` cluster. The talk and slide deck are linked at the top
of this README.
...
**A. Dev container / Codespace (zero host setup).** "Reopen in Container" (VS Code) or a GitHub
Codespace ships `uv`, `kind`, `kubectl`, `argo`, and Docker-in-Docker pinned to the versions the
cluster runs, letting you skip everything below. See
[Dev container / Codespaces](#dev-container--codespaces) for sizing and caveats.
...
maintainer-equivalent environment.

> [!NOTE]
> Verified on a **host install** and the **local dev container** (VS Code "Reopen in Container").
> Codespaces *should* behave the same (same image, same pins) but isn't tested yet; fall back to
> the [host install](#prerequisites) or the local dev container if it misbehaves.
...
"average laptop" (rung 0 runs on a 2-core free tier Codespace).
...
extension is not part of this demo.
...
See [`CONTRIBUTING.md`](./CONTRIBUTING.md). Questions go in
...
This repo was built with AI assistance (Claude Code); every change is reviewed by the maintainer.
