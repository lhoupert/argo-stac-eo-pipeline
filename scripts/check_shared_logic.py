#!/usr/bin/env python
"""Shared-logic invariant guard.

The ladder's central teaching claim: there is ONE unit of work, and every rung uses it
...
# Frozen rung-1 form of the unit of work. ingest.py is byte-stable from rung 1; if it must
# ever change, that is a deliberate, human-approved act — update this hash in the same commit.
EXPECTED_INGEST_SHA256 = "a017db32346e6f0d0dd2243ceb7bfbb6167da195cc0ad6c4bc03cade0014ef9b"
...
def ingest_drift(repo_root: Path) -> str | None:
    ...
    if actual != EXPECTED_INGEST_SHA256:
        return (
            f"ingest.py drifted from its frozen rung-1 form: "
