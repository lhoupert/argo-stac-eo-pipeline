"""The rung-1 acceptance smoke + cold/warm budget. Contract layer.

This is the central success-criteria contract made executable: a fresh clone must reach a
...
unconditional once the test runs). CI's kind-smoke job sets the env var.

Budgets: cold < 15 min, warm < 5 min. The test enforces the COLD ceiling — the hard line
...
def test_stage01_cold_smoke_within_budget() -> None:
    ...
    # THE CONTRACT. Do not weaken without explicit human approval.
    assert total_s < COLD_BUDGET_S, (
        f"cold rung-1 took {total_s:.0f}s, over the {COLD_BUDGET_S}s budget — "
        f"decide (optimize vs. relax the claim), do not silently absorb"
    )
