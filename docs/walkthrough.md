...
> are correct, but the thumbnail tile is empty. That's a known limitation: the asset hrefs
...
`INGEST_SLEEP=2s` to stand in for real IO cost), and the catalog filling to 30 items
...
Coming straight from rung 2? The backfill already filled the whole window, so there are no holes
left to find — run `make reset` (clear + re-seed) instead of `make seed`.
...
This completes the ladder's two levels of self-correction: a single run that fails transiently is
retried by Argo (rung 1, item level); a whole day that never landed is detected and refilled by
the logbook (this rung, system level).
...
It's the "here's where this goes" payoff, not a prerequisite for walking the ladder, and not part
of this demo.
...
previews, slow first `make up`, and the Windows/WSL2 note.
