...
Two backends, switched by ``config.source_type`` — a thin branch, not a plugin registry:
...
* ``synthetic`` — the ladder default: deterministic, offline items from the in-repo generator
  ...
  downloader will write to, so the item is self-consistent before any I/O happens.
...
def _asset_key(collection: str, day: date, name: str) -> str:
    """Deterministic S3 object key for an asset — shared contract with the downloader."""
