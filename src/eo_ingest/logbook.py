"""The STAC catalog as the pipeline's logbook.

Born in the foundation with ``register()``; it grows ``find_gaps()`` at rung 3 — the
...
def ensure_collection(config: Config, collection: dict) -> str:
    ...
    Same upsert shape as :func:`register`, against the collections endpoint.
    """
...
def find_gaps(
    ...
    *,
    ...
    The logbook turning active: rung 3 fans out ``ingest`` over exactly these days. Queries
